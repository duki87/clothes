    ...
    $lastTotal = 0;
      while($x = mysqli_fetch_assoc($thisYrQ)) {
        ...
        if(!array_key_exists($month, $current)) {
          $current[(int)$month] = $x['grand_total']; //For example, if month september is not in $current array, then $current = that month grand total
        } else {
          $current[(int)$month] += $x['grand_total']; //For example, if month september is already in $current array, then it will add grand_total to $current
        }
      ...
      while($y = mysqli_fetch_assoc($lastYrQ)) {
        ...
        if(!array_key_exists($month, $current)) {
          $last[(int)$month] = $y['grand_total']; //For example, if month september is not in $current array, then $current = that month grand total
        } else {
          $last[(int)$month] += $y['grand_total']; //For example, if month september is already in $current array, then it will add grand_total to $current
        }
  ...
  <?php
    ...
    while($product = mysqli_fetch_assoc($iQuery)) {
      ...
      foreach($sizes as $size) {
        if($size['quantity'] <= $size['treshold']) {
        $cat = get_category($product['categories']);
        $item = array(
          'title'     => $product['title'],
          'size'      => $size['size'],
          'quantity'  => $size['quantity'],
          'treshold'  => $size['treshold'],
